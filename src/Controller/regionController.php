...
class regionController extends Controller
{
    ...
    public function get_provinceAction() {
        ...
        if ($province) {
            ...
            $result = $this->_model('data')->getProvinceByName($province);
        }
    ...
    public function get_cityAction() {
        ...
        $province_id = isset($_GET['to_province_id']) ? $_GET['to_province_id'] : '';
        $city = isset($_GET['city']) ? $_GET['city'] : '';

        if ($province_id && $city) {

            $result = $this->_model('data')->getCityByNameAndProvinceId($province_id, $city);
        }
...
//            var_dump($sql);
            ...
            $price['lowest_self_price'] = $data['lowest_self_price'];
            $price['is_self'] = $is_self;

            // 自提总价与最低自提比较
            if($data['item_self_price']*$price['new_weight']>$data['lowest_self_price']){
                $price['self_total_price']=$data['item_self_price']*$price['new_weight'];
            }else{
                $price['self_total_price']=$data['lowest_self_price'];
            }
            // 派送总格与最低派送比较
            if($data['item_send_price']*$price['new_weight']>$data['lowest_price']){
                $price['send_total_price']=$data['item_send_price']*$price['new_weight'];
            }else{
                $price['send_total_price']=$data['lowest_price'];
            }
            // 根据类型计算总价
            if($is_self==0){
                $price['send_price'] = $data['item_self_price'] + $data['item_send_price'];//派送单价
                $price['total_price'] = $price['self_total_price']+$price['send_total_price'];//派送总价
            }else{
                $price['send_price'] = $data['item_self_price'];//自提单价
                $price['total_price']=$price['self_total_price'];//自提总价
            }
            ...
            exit(json_encode(array('errorCode'=>0, 'price'=>$price)));
